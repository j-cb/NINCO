...
timm_models = {
    ...
    'BiT_s': {'config':{'model_name': 'resnetv2_101x1_bitm', 'checkpoint_path': './model_weights/checkpoints/BiT-S-R101x1.npz'}},
    ...
    'deit3_base_patch16_224_in21ft1k': {'config':{'model_name': 'deit3_base_patch16_224_in21ft1k', 'pretrained': True}},
    ...
    'vit_base_patch16_384_laion2b_in12k_in1k':{'config':{'model_name': 'vit_base_patch16_clip_384.laion2b_ft_in12k_in1k', 'pretrained': True}},
    'vit_base_patch16_384_laion2b_in1k': {'config': {'model_name': 'vit_base_patch16_clip_384.laion2b_ft_in1k', 'pretrained': True}},
    'vit_base_patch16_384_openai_in12k_in1k': {'config': {'model_name': 'vit_base_patch16_clip_384.openai_ft_in12k_in1k', 'pretrained': True}},
    'vit_base_patch16_384_openai_in1k': {
        'config': {'model_name': 'vit_base_patch16_clip_384.openai_ft_in1k', 'pretrained': True}},
    'vit_base_patch16_384': {'config': {'model_name': 'vit_base_patch16_384.augreg_in1k', 'pretrained': True}, 'batch_size': 128,'server':'curie'},
...
NINCO_OOD_unit_tests_class_names = ['Gaussian', 'Rademacher', 'black', 'blobs', 'grey', 'horizontal_stripes', 'low_freq', 'low_freq_channelfullscale', 'low_freq_colorrange', 'low_freq_pixel_perm', 'monochrome', 'pixel_perm', 'tricolour', 'tricolour_primary', 'uni', 'vertical_stripes', 'white']
...
def extract_features(model, dataset_in_train, savepath, wo_head=False):
    ...
    slice_length = 50000
    n_slices = -((-len(dataset_in_train)) // slice_length)
    index_slices = {}
    slice_datasets = {}

    for i in range(n_slices):
        # check if current iterate is already saved in directoy
        ...
        if not complete:
            ...
            slice_datasets[
                i].__name__ = f'slice{index_slices[i].start}_to_{index_slices[i].stop}'
            slice_datasets[i].classes = dataset_in_train.classes
            ...
            with torch.no_grad():
                for (x, label) in tqdm(dataloader):
                    ...
                    feat_batch = feat_batch_preact[:,0] if wo_head else model.forward_head(feat_batch_preact, pre_logits = True)
...
def extract_clip_embeddings(model, dataloader, text=None, savepath='/scratch/mmueller67/ood_tests/shifthappens/cache_train/VisionTransformer', k=250):
    torch.backends.cudnn.benchmark = True
    # save
    if not os.path.exists(savepath):
        os.makedirs(savepath)
    ...
    labels_true = torch.cat(labels_true).numpy()
    features = np.concatenate(features, axis = 0)
    predictions_dict = {'features_{}'.format(i): features, 'labels_true_{}'.format(i): labels_true}
    ...
    if text is not None:
        ...
        predictions_dict['text_encoded']=text_encoded.cpu().numpy()
